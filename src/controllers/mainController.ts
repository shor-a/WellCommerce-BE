...
import { NotesTable } from "../config/db/schema.ts"
...
export async function getNote(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    const note = await db.select().from(NotesTable).where(eq(NotesTable.id, +req.params.id!))
...
export async function deleteNote(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    const note = await db.delete(NotesTable).where(eq(NotesTable.id, +req.params.id!)).returning({
...
export async function updateNote(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    const note = await db.update(NotesTable).set(req.body).where(eq(NotesTable.id, +req.params.id!)).returning()
